...
/**
 * Turn the page cache off.
 *
 * Automad serves cached HTML and only re-checks for content changes every
 * `AM_CACHE_MONITOR_DELAY` seconds (120 by default), so once a page has been
 * rendered, an edit can stay invisible to visitors for two minutes. For a test
 * instance that is pure flakiness: any assertion about the *rendered* page
 * becomes a race against the cache, and clearing it per request only helps
 * when the timing happens to line up.
 */
function disableCache(): void {
  log("disabling the page cache (AM_CACHE_ENABLED=0) …");
  compose(
    ["exec", "-T", SERVICE, "php", "/app/automad/console", "config:set", "--key", "AM_CACHE_ENABLED", "--value", "0"],
    { quiet: true },
  );
}
