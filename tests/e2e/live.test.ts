...
describe.skipIf(!enabled)('live E2E (real Automad v2)', () => {
  ...
  it('reports a healthy site', async () => {
    const { data } = parse(
      await client.callTool({ name: 'automad_site', arguments: { action: 'health' } }),
    );
  ...
  it('runs the full page lifecycle: create → get → delete', async () => {
    ...
    const got = parse(
      await client.callTool({ name: 'automad_pages', arguments: { action: 'get', url } }),
    );
    expect(got.isError).toBe(false);

    const deleted = parse(
      await client.callTool({ name: 'automad_pages', arguments: { action: 'delete', url } }),
    );
  ...
  it('searches the offline docs regardless of the live backend', async () => {
    ...
    const results =
      data && typeof data === 'object' && 'results' in data ? data.results : undefined;
