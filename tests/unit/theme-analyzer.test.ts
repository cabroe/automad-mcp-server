...
describe('ThemeAnalyzer', () => {
  ...
  it('omits invalid locale JSON while retaining analyzer issue', async () => {
    ...
    expect(result.issues).toContainEqual(
      expect.objectContaining({ code: 'I18N_JSON_INVALID', path: 'i18n/de.json' }),
    );
  });
  ...
  it('emits the page-data template+theme info hint when root templates exist', async () => {
    ...
    expect(
      result.findings.find((finding) => finding.code === 'PAGE_DATA_TEMPLATE_REQUIRED')?.severity,
    ).toBe('info');
  });
...
describe('main snippet detection', () => {
  ...
  it('stays silent when the snippet is defined outside the scanned buckets', async () => {
    ...
    expect(
      result.findings.some(
        (f) => f.code === 'FIELD_NOT_MASKED' && f.message.includes('snippetOnlyField'),
      ),
    ).toBe(false);
  });
  ...
  it('caps oversized snippets/ sources silently while still warning for scanned templates', async () => {
    ...
    const result = await new ThemeAnalyzer({
      fs: new LocalThemeFs(),
      themesPath: themes,
      maxSourceBytes: 32,
    }).validate('cap');
