...
describe('ThemeSchemaBuilder', () => {
  it('maps Automad field prefixes, scopes, and sources', () => {
    ...
    expect(result.warnings).toContainEqual(
      expect.objectContaining({ code: 'UNKNOWN_FIELD_PREFIX', field: 'brand' }),
    );
  });
  ...
  it('prefers shared scope and reports mask conflicts', () => {
    const input = analysis({
      fields: ['textIntro'],
      fieldSources: { textIntro: [] },
      masks: { page: ['textIntro'], shared: ['textIntro'] },
    });
    const result = new ThemeSchemaBuilder().build(input);
    expect(result.fields[0]?.scope).toBe('shared');
    expect(result.warnings).toContainEqual(
      expect.objectContaining({ code: 'FIELD_SCOPE_CONFLICT', field: 'textIntro' }),
    );
  });
  ...
  it('projects metadata and sorts ordered fields first', () => {
    ...
    const result = new ThemeSchemaBuilder().build(input);
    expect(result.fields.slice(0, 2).map((field) => field.name)).toEqual([
      'selectLayout',
      'textIntro',
    ]);
    expect(result.fields.find((field) => field.name === 'selectLayout')).toMatchObject({
      order: 0,
  ...
  it('warns for invalid field metadata and projects analyzer issues', () => {
    const input = analysis({
      ...
      manifests: {
        theme: {
          labels: { textIntro: 42 },
          options: { textIntro: ['bad'] },
          tooltips: { textIntro: false },
        },
      },
      ...
      translations: {
        de: {
          ...
          data: {
            ...
            options: {
              selectColorTheme: {
                switcher: 'Theme-Switcher anzeigen',
                light: 'Hell',
                dark: 'Dunkel',
              },
            },
  ...
  it('retains valid siblings when individual values are invalid and reports warnings', () => {
    ...
    const codes = result.warnings
      .filter((warning) =>
        ['INVALID_I18N_LABEL', 'INVALID_I18N_OPTIONS', 'INVALID_I18N_TOOLTIP'].includes(
          warning.code,
        ),
      )
  ...
  it('preserves unknown translated fields with a warning', () => {
    ...
    expect(result.warnings).toContainEqual(
      expect.objectContaining({ code: 'UNKNOWN_TRANSLATION_FIELD', field: 'mysteryField' }),
    );
  });
    ...
    const input = analysis({
      translations: {
        ...
        en: {
          locale: 'en',
          path: 'i18n/en.json',
          data: 'wrong' as unknown as Record<string, unknown>,
        },
      },
    });
    const result = new ThemeSchemaBuilder().build(input);
