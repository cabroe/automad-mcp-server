...
describe('CLAUDE.md ↔ code drift', () => {
  ...
  it('destructive-action count in CLAUDE.md matches DESTRUCTIVE_ACTIONS set', () => {
    ...
    const expected = new RegExp(
      `\\b(${n}|${numberWord(n)})\\b(?:<!--[^>]*-->)?\\s+destructive`,
      'i',
    );
  ...
  it('the AUTOGEN tools table lists every registry tool exactly once', () => {
    ...
    for (const cap of CAPABILITY_REGISTRY) {
      ...
      for (const [action] of callableActions(cap)) {
        expect(block, `missing action ${cap.name}.${action} in AUTOGEN block`).toContain(
          `\`${action}\``,
        );
      }
      for (const [action, meta] of Object.entries(cap.actions)) {
        ...
        if (meta.internal)
          expect(block, `internal ${action} leaked into AUTOGEN block`).not.toContain(
            `\`${action}\``,
          );
      }
    }
...
describe('GitHub Pages homepage ↔ code drift', () => {
  it('the AUTOGEN tool table lists every tool with its callable actions', () => {
    ...
    for (const cap of CAPABILITY_REGISTRY) {
      expect(block, `missing tool ${cap.name} on the homepage`).toContain(
        `<code>${cap.name}</code>`,
      );
      for (const [action] of callableActions(cap)) {
        expect(block, `missing action ${cap.name}.${action} on the homepage`).toContain(
          `<code>${action}</code>`,
        );
      }
      for (const [action, meta] of Object.entries(cap.actions)) {
        if (meta.internal)
          expect(block, `internal ${action} leaked onto the homepage`).not.toContain(
            `<code>${action}</code>`,
          );
      }
    }
