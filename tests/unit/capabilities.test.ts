...
function patched(name: string, change: Partial<CapabilityDefinition>): CapabilityDefinition[] {
  return CAPABILITY_REGISTRY.map((entry) =>
    entry.name === name ? { ...entry, ...change } : entry,
  );
}
...
describe('capability registry', () => {
  ...
  it('keeps internal, guard-only actions out of the callable set', () => {
    ...
    expect(callableActions(getCapability('automad_pages')).map(([name]) => name)).not.toContain(
      'update_rename',
    );
    expect(Object.keys(getCapability('automad_site').actions)).toContain('search_replace');
    expect(callableActions(getCapability('automad_site')).map(([name]) => name)).not.toContain(
      'search_replace',
    );
  });
  ...
  it('rejects duplicate names and duplicate prefixes', () => {
    expect(() =>
      validateCapabilityRegistry([...CAPABILITY_REGISTRY, CAPABILITY_REGISTRY[0]!]),
    ).toThrow('Duplicate capability name: automad_pages');
    const clashingPrefix = [
      ...CAPABILITY_REGISTRY,
      { ...CAPABILITY_REGISTRY[0]!, name: 'automad_pages' },
    ];
  ...
  it('rejects contradictory and malformed action flags', () => {
    const contradictory = patched('automad_pages', {
      actions: {
        ...getCapability('automad_pages').actions,
        list: { readOnly: true, destructive: true, description: 'List' },
      },
    });
    ...
    const readOnlyInternal = patched('automad_pages', {
      actions: {
        ...getCapability('automad_pages').actions,
        peek: { readOnly: true, destructive: false, internal: true, description: 'Peek' },
      },
    });
  ...
  it('rejects empty router and action descriptions', () => {
    ...
    const emptyAction = patched('automad_pages', {
      actions: {
        ...getCapability('automad_pages').actions,
        list: { ...getCapability('automad_pages').actions.list!, description: '' },
      },
    });
    expect(() => validateCapabilityRegistry(emptyAction)).toThrow(
      'Capability action description must not be empty: automad_pages.list',
    );
  });
});
