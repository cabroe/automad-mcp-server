...
beforeEach(async () => {
  ...
  await fs.writeFile(path.join(starter, "theme.json"), JSON.stringify({ name: "Starter", version: "0.1.0", author: "Marc" }));
...
describe("theme resources", () => {
  ...
  it("returns discovered themes with manifest metadata", async () => {
    ...
    await fs.writeFile(path.join(themes, "alpha", "theme.json"), JSON.stringify({ name: "Alpha", version: "2.0.0", author: "Ada" }));
    ...
    expect(result.themes.find((entry) => entry.slug === "alpha")?.manifest).toEqual({ name: "Alpha", version: "2.0.0", author: "Ada" });
  ...
  it("returns theme schema for a known theme", async () => {
    ...
    expect(result).toMatchObject({ theme: "starter", fields: expect.any(Array), warnings: expect.any(Array) });
  });

  it("rejects invalid slugs and missing themes with NOT_FOUND", async () => {
    await expect(readThemeSchema({ themesPath: themes }, "../escape")).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(readThemeSchema({ themesPath: themes }, "with/slash")).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(readThemeSchema({ themesPath: themes }, "")).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(readThemeSchema({ themesPath: themes }, "missing")).rejects.toMatchObject({ code: "NOT_FOUND" });
  });
