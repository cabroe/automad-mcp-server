...
beforeEach(async () => {
  ...
  await nodeFs.writeFile(path.join(starter, "theme.json"), JSON.stringify({
    name: "Starter Kit",
  ...
  await nodeFs.writeFile(path.join(starter, "package.json"), JSON.stringify({
...
describe("scaffold", () => {
  it("copies starter kit, rewrites theme.json and package.json", async () => {
    ...
    const pkg = JSON.parse(await fs.readFile(path.join(out.path, "package.json"))) as Record<string, unknown>;
