...
describe("handleDocs", () => {
  ...
  it("search requires a query", async () => {
    await expect(handleDocs({ action: "search" }, guard())).rejects.toMatchObject({ code: "VALIDATION" });
  });

  it("get requires a slug", async () => {
    await expect(handleDocs({ action: "get" }, guard())).rejects.toMatchObject({ code: "VALIDATION" });
  });
