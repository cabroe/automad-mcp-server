...
describe('handleDiscover', () => {
  ...
  it('list rejects an unknown tool filter', async () => {
    await expect(
      handleDiscover({ action: 'list', tool: 'automad_nope' }, guard()),
  ...
  it('describe requires a tool', async () => {
    await expect(handleDiscover({ action: 'describe' }, guard())).rejects.toMatchObject({
      code: 'VALIDATION',
    });
  });
  ...
  it('describes itself — every registered tool has a schema', async () => {
    const out = (await handleDiscover(
      { action: 'describe', tool: 'automad_discover' },
      guard(),
    )) as {
      tool: string;
      requires: string;
