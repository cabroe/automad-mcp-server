    ...
    ({ title, parent, summary }) =>
      userMessage(
        [
          ...
          summary
            ? `2. Call action \`update\` on the new page URL, setting \`fields.text\` to a body that opens with: "${summary}".`
            : '2. Call action `update` on the new page URL to add the body via `fields`.',
