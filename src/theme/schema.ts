...
export class ThemeSchemaBuilder {
  build(analysis: ThemeAnalysis): ThemeSchema {
    ...
    const warnings: ThemeSchemaWarning[] = analysis.issues.map(({ code, message }) => ({
      code,
      message,
    }));
    ...
    const fields = analysis.fields.map((name) => {
      ...
      if (inPage && inShared)
        warnings.push({
          code: 'FIELD_SCOPE_CONFLICT',
          message: `Field '${name}' is listed in both page and shared masks`,
          field: name,
        });
      ...
      if (label !== undefined) {
        if (typeof label === 'string') field.label = label;
          ...
          warnings.push({
            ...
            message: `Label for '${name}' must be a string`,
            field: name,
          });
      }
      ...
      if (fieldOptions !== undefined) {
        if (isStringRecord(fieldOptions)) field.options = { ...fieldOptions };
          ...
          warnings.push({
            ...
            message: `Options for '${name}' must be an object of strings`,
            field: name,
          });
      }
      ...
      if (tooltip !== undefined) {
        if (typeof tooltip === 'string') field.tooltip = tooltip;
          ...
          warnings.push({
            code: 'INVALID_FIELD_TOOLTIP',
            message: `Tooltip for '${name}' must be a string`,
            field: name,
          });
      }
...
function fieldType(name: string, warnings: ThemeSchemaWarning[]): ThemeFieldType {
  ...
  warnings.push({
    ...
    message: `Field '${name}' has no recognized Automad field prefix; defaulting to text`,
    field: name,
  });
  return 'text';
}
...
function buildTranslations(
  ...
  for (const locale of locales) {
    ...
    if (!data) {
      warnings.push({
        code: 'INVALID_I18N_ROOT',
        message: `Locale '${locale}': translation root must be an object`,
      });
      continue;
    }
    ...
    for (const field of [...translated.keys()].sort()) {
      ...
      if (!known.has(field))
        warnings.push({
          code: 'UNKNOWN_TRANSLATION_FIELD',
          message: `Locale '${locale}': translated field '${field}' is not known by the theme schema`,
          field,
        });
    }
  }
...
function normalizeStringSection(
  ...
  for (const [field, translated] of Object.entries(section)) {
    if (typeof translated !== 'string') {
      warnings.push({
        code,
        message: `Locale '${locale}': ${key} for '${field}' must be a string`,
        field,
      });
      continue;
    }
...
function normalizeOptions(
  ...
  if (!section) {
    warnings.push({
      code: 'INVALID_I18N_OPTIONS',
      message: `Locale '${locale}': options section must be an object`,
    });
    return;
  }
  for (const [field, rawOptions] of Object.entries(section)) {
    ...
    if (!options) {
      warnings.push({
        code: 'INVALID_I18N_OPTIONS',
        message: `Locale '${locale}': options for '${field}' must be an object`,
        field,
      });
      continue;
    }
    ...
    for (const [option, translated] of Object.entries(options)) {
      if (typeof translated === 'string') valid[option] = translated;
        ...
        warnings.push({
          code: 'INVALID_I18N_OPTIONS',
          message: `Locale '${locale}': option '${option}' for '${field}' must be a string`,
          field,
        });
    }
  ...
  value.forEach((entry, index) => {
    if (typeof entry !== 'string') {
      warnings.push({
        ...
        message: `fieldOrder entry at index ${index} must be a string`,
      });
      return;
    }
    if (orders.has(entry)) {
      warnings.push({
        ...
        message: `fieldOrder contains '${entry}' more than once`,
        field: entry,
      });
      return;
    }
