...
export class ThemeAnalyzer {
  constructor(
    private readonly deps: { fs: ThemeFs; themesPath: string; maxSourceBytes?: number },
  ) {}
  ...
  async analyze(theme: string): Promise<ThemeAnalysis> {
    ...
    for (const relPath of files.i18n.filter((value) => /^i18n\/[^/]+\.json$/.test(value)).sort()) {
      ...
      if (translations[locale]) {
        issues.push({
          severity: 'warning',
          code: 'DUPLICATE_I18N_LOCALE',
          message: `Duplicate i18n locale '${locale}' at ${relPath}`,
          path: relPath,
        });
        continue;
      }
    ...
    if ((await this.deps.fs.isDirectory(path.join(themePath, 'i18n'))) && files.i18n.length === 0) {
      issues.push({
        severity: 'warning',
        code: 'I18N_DIRECTORY_EMPTY',
        message: 'i18n directory contains no JSON translations',
        path: 'i18n',
      });
    }
    ...
    for (const relPath of [...files.templates, ...files.components, ...files.blocks]) {
      ...
      if (Buffer.byteLength(source, 'utf8') > maxSourceBytes) {
        issues.push({
          ...
          message: `source exceeded ${maxSourceBytes} bytes and was truncated`,
          path: relPath,
        });
    ...
    for (const relPath of [...files.lib, ...files.other.filter((p) => p.startsWith('snippets/'))]) {
      ...
      const capped =
        Buffer.byteLength(source, 'utf8') > maxSourceBytes
    ...
    if (starterKit.detected)
      issues.push({
        severity: 'info',
        code: 'STARTER_KIT_STRUCTURE_DETECTED',
        message: 'recognized Automad Theme Starter Kit structure detected',
      });
    for (const field of blockFields)
      issues.push({
        severity: 'info',
        code: 'BLOCK_FIELD_DETECTED',
        message: `block field '${field}' detected`,
      });
    if (isBuildScriptDetected(manifests.package, files.build))
      issues.push({
        severity: 'info',
        code: 'BUILD_SCRIPT_DETECTED',
        message: 'Starter Kit build script detected',
        path: 'package.json',
      });
    return {
      theme,
      ...
      fieldSources,
      blockFields,
      mainSnippet: { definedIn: mainSnippetDefinedIn, invokedIn: mainSnippetInvokedIn },
      runtimeLangFiles,
      masks,
      starterKit,
      translations,
  ...
  async validate(theme: string): Promise<ThemeValidation> {
    ...
    if (manifest && (typeof name !== 'string' || !name.trim()))
      findings.push({
        severity: 'error',
        code: 'THEME_NAME_MISSING',
        message: 'theme.json must define a non-empty name',
        path: 'theme.json',
      });
    if (analysis.files.templates.length === 0)
      findings.push({
        severity: 'error',
        code: 'THEME_TEMPLATE_MISSING',
        message: 'theme has no root-level .php template',
        path: '.',
      });
    ...
    if (
      analysis.starterKit.detected &&
      REQUIRED_BUILD_MARKERS.some((marker) => !analysis.starterKit.markers.includes(marker))
    ) {
      findings.push({
        severity: 'warning',
        code: 'STARTER_BUILD_INCOMPLETE',
        message: 'Starter Kit structure is missing one or more build markers',
        path: '.',
      });
    }
    if (analysis.files.templates.length > 0) {
      ...
      findings.push({
        ...
        message:
          "Every page's data file must set both `theme` and `template` to be renderable — see automad_docs.get('common-pitfalls')",
        path: 'pages/*/data',
      });
    }
  ...
  private async resolveTheme(theme: string): Promise<string> {
    const themePath = assertWithinRoot(
      this.deps.themesPath,
      path.join(this.deps.themesPath, theme),
    );
    if (!(await this.deps.fs.exists(themePath)))
      throw new AutomadMcpError('NOT_FOUND', `theme '${theme}' not found at ${themePath}`);
    return themePath;
  }
  ...
  private async readManifest(
    themePath: string,
    files: ThemeAnalysis['files'],
    relPath: string,
    key: 'theme' | 'package' | 'composer',
    manifests: ThemeAnalysis['manifests'],
    issues: ThemeFinding[],
  ): Promise<void> {
    ...
    const parsed = await this.readJson(
      themePath,
      relPath,
      key === 'theme' ? 'THEME_MANIFEST_INVALID' : `${key.toUpperCase()}_JSON_INVALID`,
      issues,
    );
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed))
      manifests[key] = parsed as Record<string, unknown>;
  }
  ...
  private async readJson(
    ...
    try {
      const parsed: unknown = JSON.parse(
        await this.deps.fs.readFile(path.join(themePath, relPath)),
      );
      ...
      issues.push({
        severity: 'error',
        code: invalidCode,
        message: `${relPath} is not valid JSON object`,
        path: relPath,
      });
      return undefined;
    }
  ...
  private addMetadataFindings(analysis: ThemeAnalysis, findings: ThemeFinding[]): void {
    ...
    for (const [name, manifest] of [
      ['package', analysis.manifests.package],
      ['composer', analysis.manifests.composer],
    ] as const) {
      ...
      for (const field of ['description', 'author', 'license', 'version'] as const) {
        if (
          ...
          manifest[field] &&
          theme[field] !== manifest[field]
        ) {
          findings.push({
            severity: 'warning',
            code: `${name.toUpperCase()}_METADATA_MISMATCH`,
            message: `${name}.json ${field} differs from theme.json`,
            path: `${name}.json`,
          });
        }
      }
  ...
  private addFieldFindings(analysis: ThemeAnalysis, findings: ThemeFinding[]): void {
    ...
    const known = new Set<string>([
      ...(Array.isArray(theme?.fieldOrder)
        ? theme.fieldOrder.filter((value): value is string => typeof value === 'string')
        : []),
    ...
    for (const field of analysis.fields) {
      ...
      if (
        !analysis.masks.page.includes(field) &&
        !analysis.masks.shared.includes(field) &&
        !known.has(field)
      )
        findings.push({
          severity: 'warning',
          code: 'FIELD_NOT_MASKED',
          message: `field '${field}' is not listed in masks.page or masks.shared`,
        });
    }
    for (const field of [...analysis.masks.page, ...analysis.masks.shared]) {
      if (!analysis.fields.includes(field) && !known.has(field))
        findings.push({
          severity: 'warning',
          code: 'MASK_FIELD_UNUSED',
          message: `mask field '${field}' is not referenced by templates`,
          path: 'theme.json',
        });
    }
  }
...
function isBuildFile(rel: string): boolean {
  return (
    [
      ...
      'postcss.config.js',
      'tsconfig.json',
    ].includes(rel) || rel.startsWith('bin/')
  );
}
function markerExists(
  ...
  if (marker === 'client/styles')
    return files.client.some((file) => file.startsWith('client/styles/'));
  return (
    files.client.includes(marker) || files.build.includes(marker) || files.other.includes(marker)
  );
}
function isBuildScriptDetected(
  ...
  return Boolean(
    pkg &&
    buildFiles.includes('esbuild.js') &&
    scripts &&
    typeof scripts === 'object' &&
    !Array.isArray(scripts) &&
    (scripts as Record<string, unknown>).build === 'node esbuild.js',
  );
}
function readMasks(
  manifest: Record<string, unknown> | undefined,
  issues: ThemeFinding[],
): { page: string[]; shared: string[] } {
  ...
  for (const key of ['page', 'shared'] as const) {
    ...
    if (!Array.isArray(value) || value.some((item) => typeof item !== 'string')) {
      issues.push({
        severity: 'error',
        code: 'THEME_MASKS_INVALID',
        message: `theme.json masks.${key} must be an array of strings`,
        path: 'theme.json',
      });
      continue;
    }
